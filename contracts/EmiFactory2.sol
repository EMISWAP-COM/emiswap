...
contract EmiFactory is Ownable {
        ...
        returns (Emiswap pool)
    {
        ...
        string memory symbol1 = ERC20(address(token1)).symbol(); //token1.uniSymbol();
        string memory symbol2 = ERC20(address(token2)).symbol(); //token2.uniSymbol();

        string memory name = string(abi.encodePacked("Emiswap V1 (", symbol1, "-", symbol2, ")"));
        string memory symbol = string(abi.encodePacked("EMI-V1-", symbol1, "-", symbol2));
        ...
        //bytes memory bytecode = type(Emiswap).creationCode;
        ...
        //    arg1
            ...
            //    arg1,
            //    arg2
                //"Emi-V1-", symbol1,"-", symbol2," )"
                //abi.encodePacked(token1)
                /*abi.encodePacked("Emi ( ", symbol1," - ", symbol2, " )"), 
                abi.encodePacked("EMI- UNI - DAI ") */
            //)
        //);
